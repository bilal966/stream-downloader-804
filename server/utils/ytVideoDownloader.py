...
class YouTubeVideoDownloader:
    ...
    def __getVideoStreams(self) -> list:
        try:
            return {
                "status": True,
                "video_streams_with_audio": self.__yt.streams.filter(progressive=True, type="video",
                                                                     file_extension="mp4"),
                "video_streams_without_audio": self.__yt.streams.filter(progressive=False, type="video",
                                                                        file_extension="mp4")
            }
        except AgeRestrictedError:
            raise AgeRestrictedVideoException(msg="This video is age-restricted and cannot be download")
